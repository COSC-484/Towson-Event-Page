...
import { formatDateMMddYYYY, adjustTimeZone, formatStartEndTime, timeToInt, isYourEvent, getUserId, youAreAttending }
...
const Event = props => (
    <tr>
        ...
        <td>{displayAttending(youAreAttending(props.event.event_attending, getUserId()))}</td>
        <td>{props.event.event_numAttending}</td>
        <td>
            <Link to={"/viewEvent/" + props.event._id}>View</Link>
        </td>
        <td>
            {displayEditLink(props.event.event_userId, props.event._id)}
        </td>
    </tr>
)
